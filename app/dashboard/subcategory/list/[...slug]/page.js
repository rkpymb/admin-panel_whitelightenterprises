...
const breadcrumbItems = [
  ...
  { title: 'Sub Category', link: '/dashboard/category' }
];
    ...
    try {
     const token = Cookies.get('token');
      ...
      const response = await axios.post(
        `${apiEndpoint}/admin/sub-category-list`,
        {
          page: pageNumber,
          limit: limit,
          catId:slug
    ...
    <PageContainer>
      <div className="space-y-4">
        ...
        <div className="flex items-start justify-between">
          <Heading
            title={`Sub Category (${AllData})`}
            description={`Manage sub categories of ${slug}`}
          />
          <Link
            href={`/dashboard/subcategory/create-new/${slug}`}
